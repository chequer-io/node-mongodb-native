...
function checkSupportedServer(ismaster, options) {
    ...
    if (serverVersionHighEnough) {
        ...
        return new error_1.MongoDriverError(message);
    }
...
function performInitialHandshake(conn, options, _callback) {
    ...
    if (credentials) {
        if (!(credentials.mechanism === defaultAuthProviders_1.AuthMechanism.MONGODB_DEFAULT) &&
            ...
            callback(new error_1.MongoDriverError(`authMechanism '${credentials.mechanism}' not supported`));
            return;
        }
    }
    ...
    prepareHandshakeDocument(authContext, (err, handshakeDoc) => {
        ...
        conn.command(utils_1.ns('admin.$cmd'), handshakeDoc, handshakeOptions, (err, response) => {
            ...
            if (!response.arbiterOnly && credentials) {
                ...
                if (!provider) {
                    return callback(new error_1.MongoDriverError(`No AuthProvider for ${resolvedCredentials.mechanism} defined.`));
                }
    ...
    if (credentials) {
        if (credentials.mechanism === defaultAuthProviders_1.AuthMechanism.MONGODB_DEFAULT && credentials.username) {
            ...
            if (!provider) {
                ...
                return callback(new error_1.MongoDriverError(`No AuthProvider for ${defaultAuthProviders_1.AuthMechanism.MONGODB_SCRAM_SHA256} defined.`));
            }
            return provider.prepare(handshakeDoc, authContext, callback);
        }
        const provider = defaultAuthProviders_1.AUTH_PROVIDERS.get(credentials.mechanism);
        if (!provider) {
            return callback(new error_1.MongoDriverError(`No AuthProvider for ${credentials.mechanism} defined.`));
        }
        return provider.prepare(handshakeDoc, authContext, callback);
    }
...
function parseConnectOptions(options) {
    ...
    if (!hostAddress)
        throw new error_1.MongoDriverError('HostAddress required');
    const result = {};
    ...
    if (typeof hostAddress.socketPath === 'string') {
        ...
        throw new error_1.MongoDriverError(`Unexpected HostAddress ${JSON.stringify(hostAddress)}`);
    }
}
