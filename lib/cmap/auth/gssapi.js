...
class GSSAPI extends auth_provider_1.AuthProvider {
    auth(authContext, callback) {
        ...
        if (credentials == null)
            return callback(new error_1.MongoDriverError('credentials required'));
        ...
        makeKerberosClient(authContext, (err, client) => {
            if (err)
                return callback(err);
            if (client == null)
                return callback(new error_1.MongoDriverError('gssapi client missing'));
...
function makeKerberosClient(authContext, callback) {
    ...
    const serviceName = (_a = mechanismProperties.SERVICE_NAME) !== null && _a !== void 0 ? _a : 'qp-mongodb';
    performGssapiCanonicalizeHostName(hostAddress.host, mechanismProperties, (err, host) => {
        ...
        initializeClient(spn, initOptions, (err, client) => {
            if (err)
                return callback(new error_1.MongoDriverError(err));
